using MiniSoftware.Drawing.Metafile.Emf;
using MiniSoftware.Drawing.Metafile.Emf.Records;

namespace MiniSoftware.Drawing.Metafile.Emf.Records.ObjectRecordTypes
{
    using System.IO;
    using MiniSoftware.Drawing.Metafile.Emf.Enumerations;

    internal class EMR_EXTCREATEPEN : Record
    {
        public uint IhPen;

        public uint OffBmi;

        public uint CbBmi;

        public uint OffBits;

        public uint CbBits;

        public uint PenStyle;

        public uint Width;

        public uint BrushStyle;

        public uint ColorRef;

        public uint Hatch;

        public uint NumStyleEntries;

        public uint[] StyleEntries { get; set; } = [];

        public DIBColors Usage;

        public byte[] Bmi { get; set; } = [];

        public byte[] Bits { get; set; } = [];

        public EMR_EXTCREATEPEN()
        {
            Header = new RecordHeader { Type = RecordType.EMR_EXTCREATEPEN };
        }

        public override void Read(BinaryReader reader)
        {
            long recordStart = reader.BaseStream.Position - 8;

            IhPen = reader.ReadUInt32();
            OffBmi = reader.ReadUInt32();
            CbBmi = reader.ReadUInt32();
            OffBits = reader.ReadUInt32();
            CbBits = reader.ReadUInt32();
            PenStyle = reader.ReadUInt32();
            Width = reader.ReadUInt32();
            BrushStyle = reader.ReadUInt32();
            ColorRef = reader.ReadUInt32();
            Hatch = reader.ReadUInt32();
            NumStyleEntries = reader.ReadUInt32();

            StyleEntries = new uint[NumStyleEntries];
            for (int i = 0; i < NumStyleEntries; i++)
            {
                StyleEntries[i] = reader.ReadUInt32();
            }

            Usage = (DIBColors)reader.ReadUInt32();

            long restorePosition = reader.BaseStream.Position;

            if (CbBmi > 0 && OffBmi > 0)
            {
                reader.BaseStream.Position = recordStart + OffBmi;
                Bmi = reader.ReadBytes((int)CbBmi);
            }

            if (CbBits > 0 && OffBits > 0)
            {
                reader.BaseStream.Position = recordStart + OffBits;
                Bits = reader.ReadBytes((int)CbBits);
            }

            reader.BaseStream.Position = restorePosition;
        }

        public override void Write(BinaryWriter writer)
        {
            uint cbBmi = CbBmi == 0 ? (uint)Bmi.Length : CbBmi;
            uint cbBits = CbBits == 0 ? (uint)Bits.Length : CbBits;
            uint numStyleEntries = NumStyleEntries == 0 ? (uint)StyleEntries.Length : NumStyleEntries;

            uint fixedSizeAfterHeader = 56 + (numStyleEntries * 4);
            uint offBmi = cbBmi > 0 ? fixedSizeAfterHeader : 0;
            uint offBits = cbBits > 0 ? fixedSizeAfterHeader + cbBmi : 0;

            writer.Write(IhPen);
            writer.Write(offBmi);
            writer.Write(cbBmi);
            writer.Write(offBits);
            writer.Write(cbBits);
            writer.Write(PenStyle);
            writer.Write(Width);
            writer.Write(BrushStyle);
            writer.Write(ColorRef);
            writer.Write(Hatch);
            writer.Write(numStyleEntries);
            for (int i = 0; i < numStyleEntries; i++)
            {
                writer.Write(StyleEntries[i]);
            }
            writer.Write((uint)Usage);
            writer.Write(Bmi);
            writer.Write(Bits);
        }
    }
}
