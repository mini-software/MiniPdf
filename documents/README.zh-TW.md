...
| | .NET | Rust |
|---|---|---|
| 狀態 | 穩定、功能完整 | 實驗性、持續開發中 |
| 輸入 | XLSX、DOCX、PPTX、PDF 合併 | XLSX、DOCX |
| 介面 | .NET 程式庫、CLI、獨立 Native AOT 二進位檔 | Rust crate、CLI |
| 內容擷取 | Markdown、JSON、語意化 .NET 模型 | 尚未提供 |
| 文件 | **[開啟 .NET 指南](README.nuget.md)** | **[開啟 Rust 指南](../minipdf-rs/README.md)** |

正式環境與最廣泛的文件支援建議使用 **.NET**。需要原生 Rust 相依套件，且目前轉換範圍足夠時，可選擇 **Rust**。
...
[.NET 指南](README.nuget.md)涵蓋轉換、內容擷取、PDF 合併、自訂字型、CLI 選項與部署。
...
在 VS Code 中用 GitHub Copilot Agent 開啟乾淨的 fork 或 clone，然後執行：
...
兩條路徑都需要 Git、Python 3.10+ 與 LibreOffice。.NET 需要 .NET 9 SDK；Rust 需要 Cargo，並使用桌面版 Excel 與 Word 產生目前的主要視覺參考。流程會檢查是否有已驗證的 GitHub CLI；若沒有，則產生 PR 標題、內容、推送命令與瀏覽器連結。命令與安全規則請參閱[貢獻循環](../.github/skills/skill-minipdf-contribution/SKILL.md)。未經你的確認，它不會 commit、push 或建立 PR。
